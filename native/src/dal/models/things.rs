...
pub async fn create_media_items(media: Vec<MediaInput>) -> Vec<String> {
    let mut handles = vec![];

    for m in media {
        match Thing::get_thing(m.name.clone(), m.media_id.clone()).await {
            Some(_thing) =>{
    ...
    results.into_iter().filter(|c| {
        c.is_some()
    }).map(|x| x.unwrap()).collect()
}

pub async fn update_media_items(media: Vec<MediaInput>) -> Vec<String> {
    let mut handles = vec![];

    for m in &media {
        handles.push(async move {
            delete_complete_thing(m.name.clone(), m.media_id.clone()).await
        });
    }

    join_all(handles).await;

    create_media_items(media).await
}

pub async fn delete_media_items(data: Vec<Vec<String>>) -> Vec<String> {
    let mut handles = vec![];

    for vec_kv in data {
        handles.push(async move {
            let (name, thing_id) = (vec_kv[0].clone(), vec_kv[1].clone());
            delete_complete_thing(name, thing_id).await
        });
    }
...
pub async fn create_documents(documents: Vec<DocumentInput>) -> Vec<String> {
    let mut handles = vec![];

    for doc in documents {
        match Thing::get_thing(doc.name.clone(), doc.document_id.clone()).await {
            Some(_thing) =>{
...
pub async fn update_documents(documents: Vec<DocumentInput>) -> Vec<String> {
    let mut handles = vec![];

    for doc in &documents {
        handles.push(async move {
            delete_complete_thing(doc.name.clone(), doc.document_id.clone()).await;
        });
    }

    join_all(handles).await;

    create_documents(documents).await
}

pub async fn delete_documents(data: Vec<Vec<String>>) -> Vec<String> {
    let mut handles = vec![];
...
pub async fn create_complete_thing(name: String, user_id: String, data: HashMap<String, String>, tags: Option<Vec<String>>) -> Option<String> {
    let thing_id = format!("{}", Uuid::new_v4());
    create_thing(name, user_id, data, thing_id, tags).await
}
...
impl ThingOutput {
    pub async fn scan_les_choses(
        ...
        ThingOutput::thing_output_to_vec(things).await
    }

    pub async fn query_les_choses(
        index_name: Option<String>,
    ...
    pub async fn get_les_choses(data: Vec<Vec<String>>) -> Option<Vec<ThingOutput>> {
        ...
        let things: Vec<Thing> = DynaDB::batchget_table(String::from("Things"), thing_keys).await?;
        ThingOutput::thing_output_to_vec(things).await
    }

    async fn get_thing_data(thing_id: Option<String>) -> Option<Vec<Data>> {
    ...
    pub async fn get_thing_output(name: String, user_id: String) -> Option<ThingOutput> {
        let thing = Thing::get_thing(name, user_id).await?;
        let data = ThingOutput::get_thing_data(thing.clone().thing_id).await?;
        Some(
            ThingOutput { thing, data }
        )
    }

    async fn thing_output_to_vec(things: Vec<Thing>) -> Option<Vec<ThingOutput>> {
        let mut handles = vec![];
        ...
        let list_of_choses = res.into_iter().filter(|c| {
            c.is_some()
        }).map(|x| x.unwrap()).collect();
...
async fn create_thing(name: String, user_id: String, data: HashMap<String, String>, thing_id: String, tags: Option<Vec<String>>) -> Option<String> {
...
pub async fn delete_complete_thing(name: String, thing_id: String) -> Option<String> {
    ...
    let mut data_tbl_data = HashMap::new();
    let data = ThingOutput::get_thing_data(Some(thing_id.clone())).await?;
    ...
    match DynaDB::batchdelete_data(tables_data.clone()).await {
        Some(output) => match output.unprocessed_items {
            Some(unprocessed_items) => {
                if unprocessed_items.is_empty() {
                    Some(thing_id)
...
//        debug!("Put Thing Values: {:?}", thing_values);
        ...
        let put_response = DynaDB::put(String::from("Things"), thing_values).await;
...
//        debug!("Get User Key: {:?}", key);
        ...
        let thing: Option<Thing> = DynaDB::get(String::from("Things"), key.clone()).await;
...
//        debug!("Delete Thing Key: {:?}", key);
        ...
        let result = DynaDB::delete(String::from("Things"), key).await;
...
//        debug!("Put Data Values: {:?}", datum_values);
        ...
        let put_response = DynaDB::put(String::from("Data"), datum_values).await;
...
//        debug!("Get Data Key: {:?}", key.clone());
        ...
        let datum: Option<Data> = DynaDB::get(String::from("Data"), key).await;

        debug!("Data: {:?}", datum);

        datum
    }
    pub async fn delete_data(thing_id: String) -> Option<Vec<HashMap<String, AttributeValue>>> {

        let data = ThingOutput::get_thing_data(Some(thing_id)).await?;

        let delete_handles: Vec<HashMap<String, AttributeValue>> = data.iter()
            .map(|datum: &Data| datum.clone().key()).collect();

        DynaDB::batchdelete_table(String::from("Data"), delete_handles.clone()).await;
...
//        debug!("Delete Data Key: {:?}", key);
        ...
        let result = DynaDB::delete(String::from("Data"), key).await;
        debug!("Result: {:?}", result.clone());
