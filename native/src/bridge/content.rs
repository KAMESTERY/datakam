use crate::dal::{
    User, UserAuthData, UserGroup, UserProfile, InputData, DocumentInput,
    query, get, create, update, delete,
    DeleteData, QueryInput, ThingOutput, MediaInput,
};
...
pub fn create_document(mut cx: FunctionContext) -> JsResult<JsArray> {
    let doc_obj = cx.argument::<JsObject>(0)?
        .downcast::<JsObject>()
        .unwrap_or(JsObject::new(&mut cx));

    let topic = get_js_val!(doc_obj, cx, "Topic", JsString, cx.string(""));
    let document_id = get_js_val!(doc_obj, cx, "DocumentID", JsString, cx.string(""));
    let user_id = get_js_val!(doc_obj, cx, "UserID", JsString, cx.string(""));
    let identifier = get_js_val!(doc_obj, cx, "Identifier", JsString, cx.string(""));
    let slug = get_js_val!(doc_obj, cx, "Slug", JsString, cx.string(""));
    let publish = get_js_val!(doc_obj, cx, "Publish", JsBoolean, cx.boolean(false));

    let tags_obj = doc_obj.get(&mut cx, "Tags")?.downcast::<JsArray>().unwrap_or(cx.empty_array());
    let tags: Vec<String> = get_js_array!(tags_obj, cx, JsString, cx.string(""))
        .filter(|topic| {
            !topic.is_empty()
        }).collect();
    let filtre_visuel = get_js_val!(doc_obj, cx, "FiltreVisuel", JsNumber, cx.number(0));
    let niveau = get_js_val!(doc_obj, cx, "Niveau", JsNumber, cx.number(0));
    let langue = get_js_val!(doc_obj, cx, "Langue", JsNumber, cx.number(0));
    let title = get_js_val!(doc_obj, cx, "Title", JsString, cx.string(""));
    let body = get_js_val!(doc_obj, cx, "Body", JsString, cx.string(""));
    ...
    let document_input = DocumentInput {
        ...
        document_id,
        data: vec![
            vec!["Identifier".to_owned(), identifier],
            vec!["Slug".to_owned(), slug],
            vec!["Publish".to_owned(), publish.to_string()],
            vec!["FiltreVisuel".to_owned(), filtre_visuel.to_string()],
            vec!["Niveau".to_owned(), niveau.to_string()],
            vec!["Langue".to_owned(), langue.to_string()],
            vec!["Title".to_owned(), title],
            vec!["Body".to_owned(), body]
        ],
        tags: Some(tags),
    };
    let input_data = InputData::Documents {
        documents: vec![document_input]
    };
    dbg!(&input_data);
...
pub fn delete_document(mut cx: FunctionContext) -> JsResult<JsArray> {
    let media_obj = cx.argument::<JsObject>(0)?
        .downcast::<JsObject>()
        .unwrap_or(JsObject::new(&mut cx));
    ...
    let delete_document = DeleteData::Documents {
        data
    };

    let mut rt = Runtime::new().unwrap();
    let ids = rt.block_on(async move {
        delete(delete_document).await
    });
