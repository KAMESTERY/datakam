...
data class Document(
        ...
        val filtreVisuel: Int = FiltreVisuel.IG_WILLOW.value,
        val langue: Int = Langue.ENGLISH.value,
        val niveau: Int = Niveau.LOW.value,
        ...
        fun from(item: Map<String, AttributeValue>?): Document? {
            return item?.let {
                Document(
                        ...
                        tags = item.get(TAGS)!!.ss(),
                        score = item.get(Content.SCORE)!!.n()!!.toInt(),
                        version = item.get(Content.VERSION)!!.n()!!.toInt(),
                        createdAt = Instant.parse(item.get(Content.CREATEDAT)!!.s()),
                        updatedAt = Instant.parse(item.get(Content.UPDATEDAT)!!.s()),
                        slug = item.get(SLUG)!!.s(),
                        title = item.get(TITLE)!!.s(),
                        identifier = item.get(IDENTIFIER)!!.s(),
                        body = item.get(BODY)!!.s(),
                        publish = item.get(PUBLISH)!!.bool(),
                        filtreVisuel = item.get(FILTREVISUEL)!!.n()!!.toInt(),
                        langue = item.get(LANGUE)!!.n()!!.toInt(),
                        niveau = item.get(NIVEAU)!!.n()!!.toInt()
                )
            }
        }
    }
    ...
    fun asInput(): Map<String, AttributeValue> {
        return mapOf(
            ...
            Content.CREATEDAT to AttributeValue.builder().s(this.createdAt.toString()).build(),
            Content.UPDATEDAT to AttributeValue.builder().s(this.updatedAt.toString()).build(),
